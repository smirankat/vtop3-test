...
function cleanimg() {
	return del("app/img/dest/**/*", { force: true }); // Удаляем все содержимое папки "app/img/dest/"
}
...
function startwatch() {
	...
	watch(["app/css/*.css", "!app/css/*.min.css"], styles);

	// Мониторим файлы HTML на изменения
	watch("app/**/*.html").on("change", browserSync.reload);

	// Мониторим папку-источник изображений и выполняем images(), если есть изменения
	watch(["app/img/**/*", "!app/img/dest/**/*"], images);
}
...
//deploy task to push to gh-pages
function deploy() {
